...
cp .env.example .env
# Edit .env if needed (defaults to mock mode)
...
1. Start the backend: `cd backend && streamlit run app.py`
2. Set `VITE_USE_MOCK=false` and `VITE_API_BASE_URL=http://localhost:8501` in the root `.env`
