...
if compare_mode:
    ...
    model_options = {Config.get_model_display_name(m): m for m in Config.AVAILABLE_MODELS}
    ...
    model_display_name = st.selectbox(
        "AI Model",
        list(model_options.keys()),
        index=list(model_options.values()).index(Config.DEFAULT_MODEL) if Config.DEFAULT_MODEL in Config.AVAILABLE_MODELS else 0,
...
st.markdown("This is the prompt that will be sent to the AI model(s). You can see how your selections translate into instructions.")
...
with st.sidebar:
    ...
    temperature = st.slider(
        ...
        min_value=0.0, max_value=2.0, value=Config.DEFAULT_TEMPERATURE, step=0.1,
        help="Higher values → more creative; lower → more focused.",
    )
    
    max_retries = st.number_input(
        "Max Retries",
        min_value=0, max_value=5, value=Config.MAX_RETRIES,
    ...
    if len(selected_models) > 1:
        st.info("💡 **Tip**: Compare outputs to find the best model for your content style!")
...
def _get_user_friendly_error(error, model_name: str) -> str:
    ...
    if status_code == 401:
        ...
        return "API access forbidden. Your API key may not have permission for this model."
...
def generate_for_model(model_name: str, prompt_text: str, temp: float, max_retries: int = None, show_debug: bool = False):
    """
    ...
    for current_model in models_to_try:
        debug_info[current_model] = {"attempts": [], "used_fallback": current_model != model_name}
        attempts = 0
...
if st.button("🚀 Generate Content", type="primary"):
    ...
    if not topic.strip():
        ...
        if any(not isinstance(r, str) for r in results.values()):
            ...
            with col1:
                if st.button("📋 Copy to Clipboard"):
                    successful_results = [r for r in results.values() if not isinstance(r, str)]
                    if successful_results:
                        content_to_copy = successful_results[0][0]  # First successful result
        ...
        with st.expander("📋 Selected Parameters"):
            model_names_str = ", ".join([Config.get_model_display_name(m) for m in selected_models])
            st.markdown(f"""
            **Configuration:**
