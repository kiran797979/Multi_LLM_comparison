...
def _enhance_sales_copy(text: str) -> str:
    ...
    for line in lines:
        ...
        if any(word in stripped for word in ["save", "get", "free", "guarantee", "proven", "results"]):
            enhanced_lines.append(f"**{line.strip()}**" if not line.strip().startswith("**") else line)
        else:
...
def validate_output(content_type: str, text: str) -> list:
    ...
    if content_type == "Professional Email":
        ...
        if word_count < 10:
            warnings.append("⚠️ Email content seems too short for a professional message.")
        ...
        if "#" not in text:
            warnings.append("⚠️ Blog post could benefit from subheadings for better structure.")
    
    elif content_type == "Social Media Caption":
        if "#" not in text:
            warnings.append("⚠️ Social media caption is missing hashtags.")
        if not any(emoji in text for emoji in ["😀", "🎉", "✨", "🔥", "💯", "❤️", "👍", "🌟"]):
            warnings.append("⚠️ Social media caption could be more engaging with emojis.")
    
    elif content_type == "Product Description":
        benefit_words = ["benefit", "advantage", "solution", "improve", "enhance", "save", "increase"]
        if not any(word in lower for word in benefit_words):
            warnings.append("⚠️ Product description should emphasize benefits, not just features.")
    
    elif content_type == "Press Release":
        if "for immediate release" not in lower:
            warnings.append("⚠️ Press release should start with 'FOR IMMEDIATE RELEASE'.")
        if '"' not in text:
        ...
        if "/" not in text:
            warnings.append("⚠️ Tweet thread should have numbered tweets (e.g., 1/5, 2/5).")
        tweets = [line for line in text.split("\n") if line.strip()]
        if len(tweets) < 3:
            warnings.append("⚠️ Tweet thread should have at least 3 tweets for effectiveness.")
        ...
        cta_words = ["download", "try", "get started", "sign up", "buy now", "contact", "schedule", "book"]
        if not any(w in lower for w in cta_words):
            warnings.append("⚠️ Sales content should have a clear call-to-action.")
        if "benefit" not in lower and "solution" not in lower:
            warnings.append("⚠️ Sales content should clearly communicate benefits or solutions.")
